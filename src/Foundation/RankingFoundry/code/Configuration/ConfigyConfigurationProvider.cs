...
namespace Sitecore.Foundation.RankingFoundry.Configuration
{
    ...
    public class ConfigyConfigurationProvider : XmlContainerBuilder, IConfigurationProvider
    {
        ...
        protected virtual void LoadConfigurations()
        {
            ...
            Assert.IsNotNull(configNode, "Root Foundry config node not found. Missing Foundry.config?");

            var defaultsNode = configNode["defaults"];

            Assert.IsNotNull(defaultsNode, "Foundry <defaults> node not found. It should be under <foundry> config section.");
